...
def search_contact():
    ...
    res = model.find_contact(word) 
    view.show_book(res,text.not_find(word))
    if res:
        return True
    ...
    while True:
        ...
        match choice:
            ...
            case 4:
                ...
                view.print_message(text.new_action(new_contact[0], text.operation[0]))
            case 5:
                search_contact()
            case 6:
                if search_contact():
                    ...
                    view.print_message(text.new_action(name,text.operation[1]))
            case 7:
                if search_contact():
                    c_id = int(view.input_request(text.input_del_contact_id))
                    name = model.delete_contact(c_id)
                    view.print_message(text.new_action(name,text.operation[2]))
